...
?>
	...
	{
		$tennd = mysqli_escape_string($link,$_POST["tennd"]);
		$user = mysqli_escape_string($link,$_POST["user"]);
		$pass = mysqli_escape_string($link,$_POST["pass"]);
		$gioitinh = mysqli_escape_string($link,$_POST["gioitinh"]);
		$email = mysqli_escape_string($link,$_POST["email"]);
		$dienthoai = mysqli_escape_string($link,$_POST["dienthoai"]);
		$diachi = mysqli_escape_string($link,$_POST["diachi"]);
		...
		if($hinhanh != null)
		{
			...
			$sql = "update wp_dangnhap set hinhanh = '$hinhanh' where user='$username'";
			mysqli_query($link,$sql);
			header('location:TàiKhoản.php');
		}

		$sql = "update wp_dangnhap set tennd = '$tennd',pass = '$pass',gioitinh = '$gioitinh',email = '$email',dienthoai = '$dienthoai',diachi = '$diachi' where user='$username'";
			mysqli_query($link,$sql);
	}
?>
        <form action="" method="post" enctype="multipart/form-data">
            <table width="500" style="margin: auto;">
                <tr>>
                    <td colspan="3"><b style="font-size: 22px;color:red;padding-left:130px;">Thông tin cá
                            nhân</b><br><br></td>
                ...
                <tr>
                    <td>Name : </td>
                    <td><input type="text" name="tennd" value="<?php echo $row['tennd']; ?>"></td>
                </tr>
                <tr>
                    <td>Username :</td>
                ...
                <tr>
                    <td colspan="3"><input
                            style="background-color:green; color:white;border-color:green;margin-left:170px;font-size:18px;"
                            type="submit" name="process" value="Sửa"></td>
                </tr>
            </table>
        </form>
    </div>
